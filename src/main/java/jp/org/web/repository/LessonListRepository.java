...
public interface LessonListRepository {
	@Select("select * from lessonlist")
	List<LessonListForm> getLessonListMap();
	
	@Select("select * from lessonlist where userId=#{id}")
	LessonListForm getLessonListOne(@Param("id") String userId);

	@Insert("insert into lessonlist values(#{userId}, #{userFirstName}, #{userLastName}, #{lesson1st}, #{lesson2nd})")
	...
	@Update("update lessonlist set userFirstName = #{userFirstName}"
	...
	@Delete("delete from lessonlist where userId=#{id}")
